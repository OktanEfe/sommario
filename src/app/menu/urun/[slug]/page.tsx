          ...
          <div className="space-y-8">
              ...
              <div className="space-y-3">
                ...
                <div className="flex gap-2">
                  {[1, 2, 3, 4, 5].map((step) => (
                    <div 
                      key={step} 
                      className={`h-1 flex-1 rounded-full transition-all duration-1000 ${step <= (product.intensity ?? 0) ? 'bg-[#004A30]' : 'bg-black/[0.05]'}`} 
                    />
                  ))}
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="flex gap-2">
                  {[1, 2, 3, 4, 5].map((step) => (
                    <div 
                      key={step} 
                      className={`h-1 flex-1 rounded-full transition-all duration-1000 ${step <= (product.sweetness ?? 0) ? 'bg-[#004A30]' : 'bg-black/[0.05]'}`} 
                    />
                  ))}
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="flex gap-2">
                  {[1, 2, 3, 4, 5].map((step) => (
                    <div 
                      key={step} 
                      className={`h-1 flex-1 rounded-full transition-all duration-1000 ${step <= (product.creaminess ?? 0) ? 'bg-[#004A30]' : 'bg-black/[0.05]'}`} 
                    />
                  ))}
                </div>
              </div>
            ...
            {(product.hasCoffee !== undefined || product.hasChocolate !== undefined) && (
              <div className="flex gap-2 flex-wrap">
                {product.hasCoffee && (
                  <span className="px-4 py-2 bg-[#004A30]/5 border border-[#004A30]/10 rounded-xl text-[11px] font-medium text-[#004A30]">
                    ☕ Kahve İçerir
                  </span>
                )}
              ...
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap gap-2">
                  {product.milkOptions.map(option => (
                    <span key={option} className="px-4 py-2 bg-[#FAF9F6] border border-black/[0.03] rounded-xl text-[11px] font-medium text-black/60">
                      {option}
                    </span>
                  ))}
                </div>
